    ...
    <aside className="sidebar">
          ...
          <ul className="recent-list">
            {recent.slice(0, 4).map((post) => (
              <li key={post.id} className="recent-item">
                <Link to={`/project/${post.slug}`} className="recent-item__thumb" tabIndex={-1}>
                  {post.thumbnail_url ? (
          ...
          <div className="featured-widget">
            <Link to={`/project/${featured.slug}`}>
              {featured.thumbnail_url ? (
                ...
                <div className="post-card__media"><span className="post-card__placeholder">{"</>"}</span></div>
              )}
            </Link>
            <div className="featured-widget__body">
              ...
              <span className="recent-item__date">{formatDate(featured.published_at)}</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="widget">
        ...
        <div className="follow-grid">
          <a className="follow-btn follow-btn--fb" href="#facebook" aria-label="Facebook">
            <span className="follow-btn__icon">f</span>
            <span className="follow-btn__count">4,000 Fans</span>
          </a>
          <a className="follow-btn follow-btn--ig" href="#instagram" aria-label="Instagram">
            <span className="follow-btn__icon">◉</span>
            <span className="follow-btn__count">30,000 Followers</span>
          </a>
          <a className="follow-btn follow-btn--yt" href="#youtube" aria-label="YouTube">
            <span className="follow-btn__icon">▶</span>
